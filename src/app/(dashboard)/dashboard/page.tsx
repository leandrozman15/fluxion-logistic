...
import { Truck, Driver, Load, Hub } from "@/app/lib/types";
      ...
      <div className="bg-white p-4 rounded-xl border shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4 text-sm font-medium text-slate-500">
            ...
            <span className="h-4 w-[1px] bg-slate-200 hidden md:block"></span>
            <span className="flex items-center gap-1.5 text-green-600"><Building2 size={16} /> {hubs?.length || 0} Sedes Operativas</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
        <Card className="lg:col-span-7 border-none shadow-sm overflow-hidden h-[550px] relative bg-slate-100">
          <div className="absolute inset-0 opacity-40 pointer-events-none">
            <div className="w-full h-full bg-[url('https://placehold.co/1000x800/e2e8f0/94a3b8?text=Red+Logistica+Multisede')] bg-cover"></div>
          </div>
          ...
          {hubs?.map((hub, i) => (
            ...
            >
              <div className="bg-slate-900 text-white p-2 rounded-lg shadow-xl border-2 border-white flex flex-col items-center gap-1">
                <Building2 size={20} />
          ...
          <CardHeader className="absolute top-0 right-0 p-4 pointer-events-none">
            <Badge className="bg-white/80 text-slate-900 backdrop-blur border shadow-sm">PANORAMA NACIONAL</Badge>
          </CardHeader>
        </Card>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="space-y-3 overflow-y-auto max-h-[480px] pr-2">
            {hubs?.map((hub) => (
              <Card key={hub.id} className="border shadow-none hover:shadow-md transition-all">
