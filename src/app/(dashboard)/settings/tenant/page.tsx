...
import { 
  Loader2, Save, Map as MapIcon, Globe, ShieldCheck, 
  Building2, Phone, Camera, Image as ImageIcon, 
  User, MapPin, Info, Smartphone
} from "lucide-react";
import { Tenant, TenantSettings, Country } from "@/app/lib/types";
...
export default function TenantSettingsPage() {
  ...
  if (loading) return <div className="flex h-[60vh] items-center justify-center"><Loader2 className="animate-spin text-blue-600" /></div>;
            ...
            <div className="flex flex-col md:flex-row gap-10 items-start">
               <div className="flex flex-col items-center gap-4">
                  <Avatar className="w-32 h-32 rounded-[2rem] border-4 border-slate-50 shadow-2xl relative">
                    ...
                    {isProcessingLogo && <div className="absolute inset-0 bg-white/60 flex items-center justify-center rounded-[2rem]"><Loader2 className="animate-spin text-blue-600" /></div>}
                  </Avatar>
                  <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleLogoChange} />
                  <Button variant="outline" size="sm" className="w-full rounded-xl font-bold text-[10px] uppercase" onClick={() => fileInputRef.current?.click()} disabled={isProcessingLogo}>
           ...
           <CardContent className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="space-y-1.5">
                    ...
                    <Select value={settings.country} onValueChange={(v: Country) => setSettings({...settings, country: v})}>
