          ...
          <Card className="border-none shadow-xl rounded-[2.5rem] overflow-hidden">
             ...
             <CardContent className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   ...
                   <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-slate-400">Patente Mensualizada</Label><Input type="number" className="h-11 bg-slate-50 border-none rounded-xl" value={formData.costs?.fixed.patenteMonthly} onChange={e => setFormData({...formData, costs: {...formData.costs!, fixed: {...formData.costs!.fixed, patenteMonthly: parseFloat(e.target.value) || 0}}})} /></div>
                   <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-slate-400">GPS Satelital</Label><Input type="number" className="h-11 bg-slate-50 border-none rounded-xl" value={formData.costs?.fixed.satelliteGps} onChange={e => setFormData({...formData, costs: {...formData.costs!, fixed: {...formData.costs!.fixed, satelliteGps: parseFloat(e.target.value) || 0}}})} /></div>
                   <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-slate-400">Cochera / Administración</Label><Input type="number" className="h-11 bg-slate-50 border-none rounded-xl" value={formData.costs?.fixed.garageAdmin} onChange={e => setFormData({...formData, costs: {...formData.costs!, fixed: {...formData.costs!.fixed, garageAdmin: parseFloat(e.target.value) || 0}}})} /></div>
                   <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-slate-400">Impuestos y Habilitaciones</Label><Input type="number" className="h-11 bg-slate-50 border-none rounded-xl" value={formData.costs?.fixed.taxesHabilitations} onChange={e => setFormData({...formData, costs: {...formData.costs!, fixed: {...formData.costs!.fixed, taxesHabilitations: parseFloat(e.target.value) || 0}}})} /></div>
                   <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase text-slate-400">Amortización Mensual</Label><Input type="number" className="h-11 bg-slate-50 border-none rounded-xl" value={formData.costs?.fixed.amortization} onChange={e => setFormData({...formData, costs: {...formData.costs!, fixed: {...formData.costs!.fixed, amortization: parseFloat(e.target.value) || 0}}})} /></div>
                </div>
             </CardContent>
          </Card>
        )}
